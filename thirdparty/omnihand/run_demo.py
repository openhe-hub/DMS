...
LIGHT_RED = (0.58, 0.24, 1.0)  # vivid purple: original pink was invisible on skin/pink clothing
...
def run_model_video(model_cfg, model, video_path, bbox_save_path, video_save_path, smooth=False, device='cuda:0'):
    ...
    # OMNIHAND_SMOOTH=savgol: upstream only smooths bbox scale; smooth the raw
    # per-frame centers too, otherwise detection noise shakes every crop
    if os.environ.get('OMNIHAND_SMOOTH') == 'savgol' and fNUMS >= 9:
        from scipy.signal import savgol_filter
        for side in ('right', 'left'):
            ctr = np.stack([np.asarray(bboxes[side][f"{i}"][:2], dtype=np.float64) for i in range(fNUMS)])
            ctr = savgol_filter(ctr, 9, 2, axis=0)
            for i in range(fNUMS):
                bboxes[side][f"{i}"][:2] = ctr[i]
    imgs = []
    success, frame = videoCapture.read()
    while success:
    ...
    if smooth:
        if os.environ.get('OMNIHAND_SMOOTH') == 'savgol':
            # upstream seq_smooth is a weak 70/30 blend on verts only and never
            # touches cam translation — the main source of visible jitter
            from scipy.signal import savgol_filter
            raw_vr, raw_vl = verts_right_list.copy(), verts_left_list.copy()
            raw_cr, raw_cl = cams_right_list.copy(), cams_left_list.copy()
            win = 11 if fNUMS >= 11 else max(5, fNUMS // 2 * 2 - 1)
            if fNUMS >= win:
                verts_right_list = savgol_filter(verts_right_list, win, 3, axis=0)
                verts_left_list = savgol_filter(verts_left_list, win, 3, axis=0)
                cams_right_list = savgol_filter(cams_right_list, win, 3, axis=0)
                cams_left_list = savgol_filter(cams_left_list, win, 3, axis=0)
            np.savez_compressed(os.path.join(os.path.dirname(bbox_save_path), 'traj.npz'),
                                raw_vr=raw_vr, raw_vl=raw_vl, raw_cr=raw_cr, raw_cl=raw_cl,
                                sm_vr=verts_right_list, sm_vl=verts_left_list,
                                sm_cr=cams_right_list, sm_cl=cams_left_list)
        else:
